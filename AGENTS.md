...
- When adding, removing, renaming, or changing the meaning/default/consumer of a CE config option, update `skills/ce-setup/references/config-template.yaml`, its byte-identical `.compound-engineering/config.example.yaml` copy, the centralized `docs/guides/configuration.md` reference, and the affected consumer skill docs in the same change. Ordinary keys resolve from checkout-local `config.local.yaml`, then repo `config.yaml`, then `~/.compound-engineering/config.yaml`; the home file is the lowest layer and CE never writes it. `docs_root` belongs only in repo `config.yaml`, while `packs:` resolves only from the two repo files. Durable team instructions still belong in the project's normal agent-instructions mechanism.
