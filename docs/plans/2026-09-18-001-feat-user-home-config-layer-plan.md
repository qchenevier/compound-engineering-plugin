...
- `skills/ce-product-pulse/references/run.md` (line 5) — "re-apply the ordinary-key cascade (local then tracked)"; an ordinary value-read restatement, widens to three layers.
...
**Authority note.** This list enumerates what a scan found at planning time; it is not a closed set. The unit's real boundary is the property, not the enumeration: every sentence under `skills/` that states where an ordinary key's *value* is read gains the third layer, and the named R12a first-run sentences do not. An implementer who finds a restatement this list missed edits it under that rule and names it in the result, rather than stopping — three separate scans each found one more file, so treating the list as exhaustive is what keeps failing.
