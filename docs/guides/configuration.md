...
| [`ce-code-review`](./ce-code-review.md), [`ce-doc-review`](./ce-doc-review.md) | `cross_model_code_review_scope`, `cross_model_doc_review_scope` | How many reviewers the cross-model pass sends, set separately for each skill: `default` (today's pass) or `all`. `all` sends every reviewer that can run externally to the peer target above, with the same model and effort; the adversarial reviewer and the reviewers that cannot run externally stay on the host, and a reviewer whose external run fails runs on the host instead. `all` starts more external jobs and takes longer. `cross_model_review_mode: off` still wins unless you ask for external review in conversation, and a conversation request picks the scope for one run. The health check warns about an invalid value, which is ignored in favor of the next config layer or `default`. |
