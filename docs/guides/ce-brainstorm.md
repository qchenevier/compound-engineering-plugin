...
| `output:html` | Write the plan as a single self-contained HTML file instead of markdown. Exclusive: the artifact is `.md` or `.html`, never both. Set `brainstorm_output: html` in CE config (`config.local.yaml`, then `config.yaml`, then `~/.compound-engineering/config.yaml`) to make HTML the default. A headless or pipeline run resolves the format the same way; nothing forces markdown. See the [configuration reference](./configuration.md). |
