...
At scope `default`, this pass is **adversarial-only** (Step 1 resolves the scope). No other persona gets a cross-model twin, and there is no whole-diff generalist peer. The pass costs nothing unless Stage 3 (reviewer selection) already selected the adversarial reviewer. Scope `all` sends every eligible selected persona instead, as `references/all-reviewers-external.md` defines; there is still no whole-diff generalist peer.
...
1. At scope `default`, `adversarial-reviewer` was selected in Stage 3 (reviewer selection), or the Review depth gate chose the focused path, whose one independent read is this lens. At scope `all`, at least one selected persona is eligible under `references/all-reviewers-external.md`. Reuse that selection; do not run a costly external CLI on a trivial diff.
...
**Checkout policy on sending content out — evaluate first.** Read `cross_model_review_mode:` under the ordinary-key rule above. Valid values are `auto` (default) and `off`; anything else is invalid and continues to the next layer, then `auto`. When it resolves to `off`, skip the automatic cross-model pass here, before peer resolution, disclosure, or any job start. The one exception is when the user explicitly asked for a cross-model peer for this run in conversation; a `cross_model_peer` value or a project-instruction preference is not that opt-in. Record the skip reason as **disabled by checkout config**, which is a different reason from an un-attestable host or an unavailable or non-independent route. The in-process `adversarial-reviewer` keeps the lens exactly as it does for any peer that never started. A live user prohibition still overrides `auto`. This pass's skip and target-selection keys are `cross_model_review_mode` and `cross_model_peer`. Missing files or unset keys take the default auto route; they are not a skip. Another skill's engine preference is not this gate. Model and effort overrides stay with the bound target as the reference states.

**Review scope.** Resolve it only after the checkout policy above lets the pass run. A request in conversation for this run wins, such as "send all reviewers to codex" (`all`) or "only the usual cross-model pass this time" (`default`). Otherwise read `cross_model_code_review_scope:` under the ordinary-key rule. Valid values are `default` and `all`; anything else is invalid and continues to the next layer, then `default`. The lite and focused depth paths always run at `default` (`references/depth-paths.md`), and `pr-remote` / `branch-remote` start no job at either scope. At `default`, this reference is the whole pass. At `all`, finish the target and route resolution below first. When the resolved target is the host's own family, or no eligible route is installed, run at `default` and say why in one line. Otherwise read `references/all-reviewers-external.md` before Step 3. It replaces this reference's adversarial-only run condition, the Step 3 announcement, the per-job input file names, and the single-job start and collection, and it adds the fallback and provenance rules. Everything else here applies at both scopes.
...
The script uses the persona brief its optional fifth argument names, and the adversarial brief when that argument is absent, as it is at scope `default`; fold-in forces `reviewer` to `<persona>-<provider>`.
