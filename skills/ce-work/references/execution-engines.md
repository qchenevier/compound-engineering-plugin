...
Standing configuration uses one mode plus an ordered route list. Resolve `work_engine_mode` and `work_engine_preferences` independently under the ordinary-key rule above; a present local list, including `[]`, replaces the team list. Do not pick one file for the whole group.
