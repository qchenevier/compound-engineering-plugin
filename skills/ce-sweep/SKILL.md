...
**Route to Phase 1** when `feedback_sources` is unset in both repo layers, `config.local.yaml` and `config.yaml`, or when a `setup` or `reconfigure` token is present, whatever the config state. The home layer is read for the value but never counts toward this first-run check. Otherwise route to Phase 2. "Config keys" in `references/run.md` defines `feedback_sources` and each `sweep_*` key with its default.
