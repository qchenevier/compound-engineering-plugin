...
- **Cross-model Coverage rows**: At scope `default`, report the cross-model pass as Step 5 of `references/cross-model-review.md` states. At scope `all`, add one row per external job, named `<reviewer-name>-<provider>`, including jobs that fell back. Its Status is `completed (external, verified: <served model>)` or `completed (external, unverified: requested <model>)`. A reviewer that ran externally has only its job row, which carries its counts. A job without a usable result keeps its row with Status `host fallback: <reason>` and `--` counts, and its in-process twin's persona row carries the counts with Status `completed (host fallback)`. `feasibility` and `adversarial-document` show `completed (host by design)`. These Status values keep the four provenance states visible without adding a column.
