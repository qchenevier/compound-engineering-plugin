...
**Cross-model returns.** A `<reviewer-name>-<provider>` return merges with its in-process twin under the same one-fix test. The cross-model pass reports the review scope. At scope `default`, a return is an extra peer beside its in-process twin. At scope `all`, a return stands in for the selected reviewer it names, and that reviewer's twin runs only as a fallback (`references/all-reviewers-external.md`). Whether that merge counts as *independent corroboration* is decided in 3.4 (Cross-Persona Agreement Promotion) by the return's `independence_verified` flag, not here.
...
For local personas, independence requires separate dispatched contexts; an inline fallback cannot trigger anchor promotion. Cross-model corroboration requires `independence_verified: true`, at least one in-process contributor, and an independence-verified peer. A missing or false flag cannot trigger anchor promotion. Cursor default/Auto is not verified independence unless the run recorded which model actually answered. Peer-only agreement never promotes, and additional peers never stack the promotion. External reviewers agreeing only with each other are peer-only agreement at either scope, because they share one model family. At scope `all`, a reviewer that ran on the host's model counts as the in-process contributor, including a fallback twin.

Record any justified promotion in the Reviewer column as `(+1 anchor)`, naming the cross-model reviewer and its verified model or route legibly. Keep the stored reviewer identities. Findings dropped at anchors 0/25 do not return through agreement. Corroboration never grants permission to apply fixes: at scope `default`, the limits on peer-only findings in 3.6 (Resolve Who Can Choose the Fix) and 3.7 (Route by Autofix Class) still apply, and at scope `all`, the ordinary 3.7 routing applies.
...
An obligation that changes meaning uses `gated_auto` and must include a specific `suggested_fix`. A mechanical `safe_auto` correction keeps its class, subject at scope `default` to the restriction on findings raised only by another model. If investigation still leaves no specific edit to apply, exclude it from the group and return the missing information to the calling agent.
...
Apply such a correction at anchor `100` when a local reviewer supports the finding and a specific `suggested_fix` is ready. At scope `all`, an external stand-in's support counts as the support its in-process twin would have given. This may be `safe_auto` or `gated_auto`: changing wording or meaning to fulfill an existing decision is different from making a new decision. At scope `default`, findings raised only by another model retain the R18 restriction. Session-settled annotations remain protected at either scope.
...
**No silent fixes from another model alone (scope `default`).** At that scope, findings raised only by another model never go directly to Apply, regardless of confidence or class (R18). Show a verified, chosen fix for approval with the others. Keep `manual` when a user decision or essential information is still missing. The source of a finding limits silent application, not the lead agent's ability to investigate and recommend. At scope `all`, external stand-ins are routed as 3.6 states instead.
...
- Do **not** apply anything 3.7 routed elsewhere. Obligations and, at scope `default`, peer-only findings diverted out of Apply join the grouped confirmation; anchor `50` goes to FYI; `manual` at any anchor is a decision. If a finding reaches this step from any of those routes, 3.7 was not applied correctly. Re-run it for that finding before continuing.
- At scope `default`, do **not** apply a finding whose only reviewers are cross-model peers, at any anchor or class. 3.7 diverts those to the grouped confirmation when the table would have applied them, and keeps choices that only the user can make in the separate Decisions section after the check in 3.6 of who can choose the fix.
